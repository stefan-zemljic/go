...
func (s *MultiMap[K, V]) JsonTo(w *Writer) {
	w.Add(Object)
	remaining := maps.Clone(s.values)
	for _, key := range s.Keys() {
		w.Add(key)
		values := remaining[key]
		w.Add(values[0])
		remaining[key] = values[1:]
	}
	w.Add(End)
}
func (s *MultiMap[K, V]) Json() string {
	var w Writer
	s.JsonTo(&w)
	return w.String()
}
func (s *MultiMap[K, V]) Pretty() string {
	var w Writer
	s.JsonTo(&w)
	return w.Pretty()
}
func (s *MultiMap[K, V]) String() string {
	return s.Pretty()
}
